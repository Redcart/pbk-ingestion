...
def transform_data(input_path, bucket_name, output_path, mode, date_time):
    ...
    if mode == "stations":
        ...
        for station in range(len(raw_data.get("stations"))):
            ...
            data_one_station["state_name"] = raw_data_one_station.get("state").get(
                "name"
            )
            ...
            data_one_station["network_id"] = raw_data_one_station.get("network").get(
                "id"
            )
            data_one_station["network_name"] = raw_data_one_station.get("network").get(
                "name"
            )
            data_one_station["is_virtual_station"] = raw_data_one_station.get(
                "is_virtual_station"
            )
        ...
        for station in range(len(raw_data.get("stations"))):
            ...
            for bike in range(len(raw_data_one_station.get("vehicles"))):
                ...
                data_one_bike["vehicle_ebike_battery_level"] = raw_data_one_bike.get(
                    "ebike_battery_level"
                )
                data_one_bike["vehicle_type_id"] = raw_data_one_bike.get("type").get(
                    "id"
                )
                data_one_bike["vehicle_type_name"] = raw_data_one_bike.get("type").get(
                    "name"
                )
...
def ingest_data(input_path, bucket_name, project_id, dataset, table, mode):
    ...
    if mode == "stations":
        ...
        df_transformed["ingestion_time"] = pd.to_datetime(
            df_transformed["ingestion_time"], format="%Y-%m-%d %H:%M:%S"
        )
